...
### 1. 담당 기능 Full-Stack 개발 (End-to-End)
- **도메인 전담 개발**: 서비스 내 **이미지 관리 시스템** 및 **숙소/객실 관리** 도메인을 전담하여 DB 설계, API 구현, UI/UX 개발까지 전 과정을 수행했습니다.
- **성능 및 구조 최적화**: 
  - 이미지 업로드 시 Base64 방식의 성능 저하 문제를 서버 직접 저장 방식으로 개선했습니다.
  - 대량 데이터 조회 시 발생하는 **N+1 쿼리 문제**를 배치 로딩(Batch Loading) 기법으로 해결했습니다.

### 2. CI/CD 파이프라인 및 인프라 구축
- **배포 자동화**: **GitHub Actions**와 **AWS EC2**를 연계한 CI/CD 파이프라인을 설계하여 코드 푸시부터 배포까지의 과정을 자동화했습니다.
- **컨테이너 환경 운영**: **Docker**를 도입하여 개발 및 운영 환경의 일관성을 확보하고 시스템 관리 효율성을 높였습니다.
